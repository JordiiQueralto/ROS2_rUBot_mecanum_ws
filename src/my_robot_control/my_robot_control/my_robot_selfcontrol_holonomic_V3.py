...
class RobotSelfControl(Node):
    ...
    def __init__(self):
        ...
        # Memory of last direction
        self._last_direction = Twist()
        self._last_direction.linear.x = self._msg.linear.x
        self._last_direction.linear.y = self._msg.linear.y
        self._last_direction.angular.z = self._msg.angular.z
    ...
    def timer_callback(self):
        ...
        if now_sec - self._last_speed_time >= 1:
            self.get_logger().info(f"Vx: {self._msg.linear.x:.2f} m/s, Vy: {self._msg.linear.y:.2f}, w: {self._msg.angular.z:.2f} rad/s | Time: {elapsed_time:.1f}s")
    ...
    def laser_callback(self, scan):
        ...
        if -45 <= angle_closest_distance <= 45:
            ...
            zone = "BACK_RIGHT"
        elif (-150 > angle_closest_distance >= -180) or (150 < angle_closest_distance <= 180):
            zone = "BACK"
        else:
            zone = "OUTSIDE FOV"
        ...
        if closest_distance < self._distanceLimit:
            if zone == "FRONT":
                ...
                self._msg.linear.x = 0.0
                self._msg.linear.y = 0.0
                self._msg.angular.z = self._forwardSpeed * 5.0 * self._speedFactor
            ...
            # KEEP last direction instead of forcing forward motion
            self._msg.linear.x = self._last_direction.linear.x
            self._msg.linear.y = self._last_direction.linear.y
            self._msg.angular.z = self._last_direction.angular.z
