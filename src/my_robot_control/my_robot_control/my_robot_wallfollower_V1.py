...
class WallFollower(Node):
    ...
    def laser_callback(self, scan):
        ...
        if min_front < self.base_distance:
            twist.linear.x = 0.0 #No avançar 
            twist.linear.y = self.v_lin #Mou lateralment esquerra
            twist.angular.z = 0.0
            action = f"FRONT {min_front:.2f} m → turn LEFT"

        #----------------------------------------------------------
        # RULE 2: FRONT-RIGHT obstacle → mou diagonal ESQUERRA-ENDAVANT
        #----------------------------------------------------------
        elif min_fr_right < self.base_distance:
            twist.linear.x = self.v_lin * 0.5 #una mica endavant
            twist.linear.y = self.v_lin * 0.8 #principalment esquerra
            twist.angular.z = 0.0 #manté orientació
            action = f"FRONT-RIGHT {min_fr_right:.2f} m → move LEFT-FORWARD"

        #----------------------------------------------------------
        # RULE 3: RIGHT visible → control with tolerance band (no vy) (control per mantenir distància)
        #----------------------------------------------------------
            ...
            if abs(error) <= self.tol:
                ...
                twist.angular.z = 0.0
                action = (
                    f"RIGHT ~OK ({min_right:.2f}m) → FORWARD"
                )
                ...
                twist.angular.z = 0.0
                action = (
                    f"RIGHT CLOSE ({min_right:.2f}m) → LEFT-FORWARD"
                )
                ...
                twist.angular.z = 0.0 
                action = (
                    f"RIGHT FAR ({min_right:.2f}m) → RIGHT-FORWARD"
                )

        #----------------------------------------------------------
        # RULE 4: BACK-RIGHT → only if it is the most relevant wall, mou diagonal dreta-endavant
        #----------------------------------------------------------
            ...
            twist.angular.z = 0.0 
            action = (
                f"BACK-RIGHT {min_back_right:.2f} m → move FORWARD-RIGHT"
            )

        #----------------------------------------------------------
        # RULE 5: BACK osbtacle  → mou lateralment DRETA (Y-)
        #----------------------------------------------------------
        elif math.isfinite(min_back) and min_back < self.base_distance:
            twist.linear.x = 0.0  # No avançar
            twist.linear.y = -self.v_lin  # Mou lateralment DRETA
            twist.angular.z = 0.0  # MANTÉ ORIENTACIÓ
            action = f"BACK {min_back:.2f} m → move RIGHT (holonomic)" 
        
        #----------------------------------------------------------
        # RULE 6: Si no detecta res, busca la paret avançant i dreta
        #----------------------------------------------------------
        else:
            twist.linear.x = self.v_lin * 0.5  # Avança a mitja velocitat
            twist.linear.y = -self.v_lin * 0.5  # Es mou una mica a la dreta
            twist.angular.z = 0.0  # MANTÉ ORIENTACIÓ
            action = "No wall detected → SEARCH (forward-right)"
